...
- The browser does not talk directly to the processing service during normal playback.
- Real-time progress is delivered through the status service WebSocket, not polling.
- Playback depends on both the streaming service and a browser-reachable `MINIO_PUBLIC_ENDPOINT`.
- A video must reach `READY` before playback endpoints succeed.
